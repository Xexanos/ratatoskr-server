...
// served side by side. The Fastify mount (app.ts's openapi-glue `prefix`) and the cover URL built
// into that major's library projection (abs/client.ts) both come from here, so the mounted routes,
// the URLs the API hands out, and the contract that documents them cannot drift apart — deriving it
// is what makes that a guarantee rather than a note.
//
